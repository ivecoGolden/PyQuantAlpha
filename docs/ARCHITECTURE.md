...
flowchart TD
    ...
    subgraph 编译层["🧠 LLM 编译层"]
        LLM[LLM 编译服务]
        IR[策略 IR<br/>JSON/DSL]
        STRCODE[策略代码草稿]
        UICODE[前端页面草稿]
    end

    subgraph 校验层["🔒 安全校验层"]
        IRValidator[IR 校验<br/>风控检查]
        StrategySanitizer[策略代码静态检查<br/>只能调用抽象API]
        UISanitizer[前端代码校验<br/>组件白名单/隔离]
    end
...
```mermaid
flowchart LR
    LLM[LLM 模型] -->|生成| Code[策略代码]
    Code -->|调用| API[runtime_api]
    API -->|执行| Trade[交易所]
    
    LLM x--x|禁止直接访问| Trade
```
...
```mermaid
flowchart TD
    A[1️⃣ 输入自然语言策略] --> B[2️⃣ 系统生成]
    B --> B1[IR]
    B --> B2[策略代码]
    B --> B3[前端页面]
    B --> B4[策略说明]
    
    B1 & B2 & B3 & B4 --> C[3️⃣ 用户调参]
    C --> D[4️⃣ 回测验证]
    D --> E[5️⃣ 模拟盘运行]
    E --> F[6️⃣ 小额实盘]
    F --> G[7️⃣ 自动运行]
