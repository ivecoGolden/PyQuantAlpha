...
- $E(R_p)$: 策略年化收益率
- $R_f$: 无风险利率
- $\sigma_d$: **下行标准差**（仅计算负收益的波动率）

**优势**: 对于偏正收益的策略更公平，不会因为大幅盈利而被惩罚。

### 卡尔玛比率 (Calmar Ratio)
衡量收益与最大回撤之间的关系，体现"冒险是否值得"。
**公式**:
$$
Calmar = \frac{年化收益率}{|最大回撤|}
$$

**用法**: Calmar > 1.0 表示年化收益超过最大回撤，通常 > 3.0 为优秀。
...
> **核心理念**: 将 "交易什么" (Strategy) 与 "交易多少" (Sizer) 解耦。

Sizer 模块负责根据策略信号和账户状态，自动计算每笔交易的仓位大小。
...
```python
# 每次买入 0.1 BTC
sizer = FixedSize(SizerParams(size=0.1))
```

**适用场景**: 简单测试、小资金账户。

### 9.2 百分比仓位 (PercentSize)
按账户可用资金的百分比计算仓位。

**参数**: `percent` (百分比，0-100)

**公式**:
$$
Size = \frac{Cash \times Percent\%}{Price}
$$
...
**公式**:
$$
Size = \frac{Cash}{Price}
$$

> [!CAUTION]
> 全仓策略风险极高，一旦亏损将无法补仓。仅适用于信号极度确定的场景。

### 9.4 风险仓位 (RiskSize)
基于风险控制（如 ATR 止损距离）计算仓位。

**参数**:
- `risk_percent`: 每笔交易最大风险占总资金比例
- `stop_distance`: 止损距离（可用 ATR 倍数）

**公式**:
$$
Size = \frac{Equity \times RiskPercent\%}{StopDistance}
$$

**示例**:
```python
# 每笔交易风险不超过 2%，止损距离为 2 倍 ATR
sizer = RiskSize(SizerParams(risk_percent=2, stop_distance=atr_value * 2))
```

**优势**: 波动大时自动减仓，波动小时自动加仓。
...
**公式**:
- 买入: $ExecutedPrice = OrderPrice + Slippage$
- 卖出: $ExecutedPrice = OrderPrice - Slippage$

#### 百分比滑点 (PercentSlippage)
按订单价格的百分比计算滑点。

**公式**:
$$
Slippage = OrderPrice \times SlippagePercent
$$
...
#### 成交量滑点 (VolumeSlippage)
考虑订单对市场的冲击成本（大单更容易滑点）。

**逻辑**: 订单量越大占当前成交量比例越高，滑点越大。

### 10.2 手续费模型 (Commission)
模拟交易所收取的交易费用。
...
| 百分比 | 按成交金额比例收费 | 0.1% |
| 固定费用 | 每笔固定金额 | $1/笔 |
| 阶梯费率 | 成交量越大费率越低 | VIP 等级 |

**公式 (百分比)**:
$$
Fee = ExecutedPrice \times Size \times CommissionRate
$$
...
### 11.1 资金费率 (Funding Rate)
永续合约特有机制，用于锚定合约价格与现货价格。

**原理**:
- 当合约价格 > 现货价格时，多头付费给空头（正费率）
- 当合约价格 < 现货价格时，空头付费给多头（负费率）

**结算周期**: 通常每 8 小时结算一次

**策略应用**:
- 持续负费率 → 空头情绪强，可能反弹机会
- 资金费率套利：同时做多现货、做空合约，赚取费率

**API 示例**:
```python
funding_rates = self.get_funding_rates("BTCUSDT", days=7)
for fr in funding_rates:
    print(f"{fr.time}: {fr.rate:.4%}")
```

### 11.2 多空比 (Long/Short Ratio)
交易所账户的多头与空头持仓比例。

**公式**:
$$
Ratio = \frac{多头账户数}{空头账户数}
$$

**解读**:
| 多空比 | 市场情绪 | 潜在信号 |
|-------|---------|---------|
| > 1.5 | 散户极度看多 | 反向指标，可能回调 |
| < 0.7 | 散户极度看空 | 反向指标，可能反弹 |
| ≈ 1.0 | 多空平衡 | 无明显偏向 |

> [!TIP]
> 多空比常作为**反向指标**：散户越看好某方向，大户越可能反向操作。

### 11.3 持仓量 (Open Interest)
未平仓合约的总量，反映市场参与热度。
...
Bracket Order
├── 主订单 (Entry): 买入/卖出入场
├── 止盈单 (Take Profit): 限价卖出
└── 止损单 (Stop Loss): 止损卖出
```

**特点**: 主订单成交后，止盈单和止损单自动激活；其中一个成交后，另一个自动取消。
...
self.buy_bracket(
    ...
    size=0.1,
    stopprice=48000,   # 止损价
    limitprice=55000   # 止盈价
)
```

### OCO 订单 (One-Cancels-Other)
两个订单互斥，当其中一个成交时，另一个自动取消。
...
**参数**:
- `trail_amount`: 固定金额回撤触发
- `trail_percent`: 百分比回撤触发

**逻辑 (多头)**:
1. 初始止损价 = 入场价 - TrailAmount
2. 价格上涨时，止损价同步上移
3. 价格下跌时，止损价保持不变
4. 价格触及止损价时平仓
