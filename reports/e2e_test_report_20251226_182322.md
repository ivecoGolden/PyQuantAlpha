...
class Strategy:
    ...
    def on_bar(self, data):
        ...
        # 更新指标
        ema_fast_val = self.ema_fast.update(bar.close)
        ema_slow_val = self.ema_slow.update(bar.close)
        rsi_val = self.rsi.update(bar.close)
        atr_val = self.atr.update(bar.high, bar.low, bar.close)
        bb_val = self.bb.update(bar.close)
        ...
        if not self.in_position:
            # 做多条件：EMA20 > EMA60（趋势向上）且 RSI < 40（超卖）且价格触及布林带下轨
            if ema_fast_val > ema_slow_val and rsi_val < 40 and bar.close <= bb_val.lower:
                # 计算止损和止盈价格
                stop_price = bar.close - 2 * atr_val
                take_profit = bar.close + 4 * atr_val
                # 使用挂钩订单入场（自动设置止损和止盈）
                self.buy_bracket(self.symbol, size=None, stopprice=stop_price, limitprice=take_profit)
                
            # 做空条件：EMA20 < EMA60（趋势向下）且 RSI > 60（超买）且价格触及布林带上轨
            elif ema_fast_val < ema_slow_val and rsi_val > 60 and bar.close >= bb_val.upper:
                # 计算止损和止盈价格
                stop_price = bar.close + 2 * atr_val
                take_profit = bar.close - 4 * atr_val
                # 使用挂钩订单入场（自动设置止损和止盈）
    ...
    def notify_order(self, order):
        ...
        if order.status == "FILLED":
            print(f"订单成交: {order.id}, 交易对: {order.symbol}, 方向: {order.side}, 价格: {order.price}, 数量: {order.quantity}")
        elif order.status == "REJECTED":
            print(f"订单被拒绝: {order.id}, 原因: {order.reason}")
