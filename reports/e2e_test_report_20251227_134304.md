...
class Strategy:
    def init(self):
        ...
        # 设置仓位管理：基于ATR风险控制，每次最大亏损2%，ATR乘数为2
    ...
    def on_bar(self, data):
        ...
        if not self.in_position:
            # 做多条件：EMA20 > EMA60（趋势向上）且 RSI < 40（超卖）且价格触及布林带下轨
            if ema20_val > ema60_val and rsi_val < 40 and close <= bb_lower:
                # 计算止损和止盈价格（基于ATR）
                stop_price = close - 2 * atr_val  # 止损距离为2倍ATR
                take_profit = close + 4 * atr_val  # 止盈距离为4倍ATR，风险回报比1:2
                
                # 使用挂钩订单买入（自动创建止损和止盈单）
                # 数量由sizer自动计算
                self.buy_bracket(self.symbol, stopprice=stop_price, limitprice=take_profit)
                
            # 做空条件：EMA20 < EMA60（趋势向下）且 RSI > 60（超买）且价格触及布林带上轨
            elif ema20_val < ema60_val and rsi_val > 60 and close >= bb_upper:
                # 计算止损和止盈价格（基于ATR）
                stop_price = close + 2 * atr_val  # 止损距离为2倍ATR
                take_profit = close - 4 * atr_val  # 止盈距离为4倍ATR，风险回报比1:2
                ...
                # 使用挂钩订单卖出（自动创建止损和止盈单）
                # 数量由sizer自动计算
                self.sell_bracket(self.symbol, stopprice=stop_price, limitprice=take_profit)
    ...
    def notify_order(self, order):
        ...
        if order.status == "FILLED":
            ...
            print(f"订单被拒绝: {order.id} - {order.symbol}")
        elif order.status == "CANCELLED":
            print(f"订单取消: {order.id} - {order.symbol}")
            
    def notify_trade(self, trade):
        # 打印交易盈亏
        if trade.pnl != 0:
            print(f"交易平仓: {trade.symbol} 盈亏: {trade.pnl:.2f} 手续费: {trade.fee:.2f}")
```
