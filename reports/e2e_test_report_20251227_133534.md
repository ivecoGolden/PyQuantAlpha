...
class Strategy:
    def init(self):
        ...
        # 设置仓位管理：基于ATR风险控制，单次最大亏损2%，ATR乘数2
    ...
    def on_bar(self, data):
        ...
        ema_fast_val = self.ema_fast.update(close_price)
        ema_slow_val = self.ema_slow.update(close_price)
        rsi_val = self.rsi.update(close_price)
        atr_val = self.atr.update(high_price, low_price, close_price)
        bb_val = self.bb.update(close_price)
        
        # 检查指标是否有效（避免初始阶段None值）
        if ema_fast_val is None or ema_slow_val is None or rsi_val is None or atr_val is None or bb_val is None:
            return
        
        # 获取当前持仓
        self.position = self.get_position(self.symbol)
        
        # 如果没有持仓，检查入场条件
        if self.position is None:
            # 做多条件：EMA20 > EMA60（趋势向上）且 RSI < 40（超卖）且价格触及布林带下轨
            if ema_fast_val > ema_slow_val and rsi_val < 40 and close_price <= bb_val.lower:
                # 使用挂钩订单：买入+止损+止盈
                stop_price = close_price - 2 * atr_val  # 止损价：当前价 - 2*ATR
                take_profit = close_price + 4 * atr_val  # 止盈价：当前价 + 4*ATR（风险回报比2:1）
                self.buy_bracket(self.symbol, size=None, stopprice=stop_price, limitprice=take_profit)
                
            # 做空条件：EMA20 < EMA60（趋势向下）且 RSI > 60（超买）且价格触及布林带上轨
            elif ema_fast_val < ema_slow_val and rsi_val > 60 and close_price >= bb_val.upper:
                # 使用挂钩订单：卖出+止损+止盈
                stop_price = close_price + 2 * atr_val  # 止损价：当前价 + 2*ATR
                take_profit = close_price - 4 * atr_val  # 止盈价：当前价 - 4*ATR（风险回报比2:1）
                self.sell_bracket(self.symbol, size=None, stopprice=stop_price, limitprice=take_profit)
    ...
    def notify_trade(self, trade):
        # 打印交易盈亏
        if trade.pnl != 0:
            print(f"交易平仓: 交易对={trade.symbol}, 盈亏={trade.pnl:.2f}, 手续费={trade.fee:.2f}, 时间={trade.timestamp}")
```
...
| 交易对 | BTCUSDT |
| K线数量 | 1440 根 |
| 时间范围 | 2025-10-28 14:00:00 ~ 2025-12-27 13:00:00 |
