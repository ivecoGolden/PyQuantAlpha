...
class Strategy:
    def init(self):
        ...
        # 设置基于ATR风险的仓位管理，单次最大亏损2%，ATR乘数为2
    ...
    def on_bar(self, data):
        ...
        # 获取布林带上下轨
        bb_upper = bb_val.upper
        bb_lower = bb_val.lower
        ...
        if position is None:
            ...
            return  # 有持仓时，不执行新入场逻辑
        
        # 入场条件判断
        # 做多条件：EMA20 > EMA60（趋势向上）且 RSI < 40（超卖）且价格触及布林带下轨
        long_condition = (ema20_val > ema60_val) and (rsi_val < 40) and (close <= bb_lower)
        
        # 做空条件：EMA20 < EMA60（趋势向下）且 RSI > 60（超买）且价格触及布林带上轨
        short_condition = (ema20_val < ema60_val) and (rsi_val > 60) and (close >= bb_upper)
    ...
    def notify_order(self, order):
        ...
        if order.status == "FILLED":
            ...
            print(f"订单取消: ID={order.id}, 交易对={order.symbol}")
